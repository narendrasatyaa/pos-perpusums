...
{{-- nota template --}}
<div class="hidden print:block" style="width: 80mm; font-family: 'Courier New', Courier, monospace; font-size: 12px; color: #000; padding: 2px;">
...
{{-- web ui --}}
<div class="min-h-screen bg-slate-50 font-sans selection:bg-accent selection:text-primary print:hidden flex flex-col">
    {{-- <!-- Top Nav -->
    ...
    <main class="flex-1 flex flex-col items-center py-12 px-4 relative">
        ...
        <!-- Receipt Card -->
        <div class="bg-white w-full max-w-[420px] rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-8 relative z-10 overflow-hidden">
            ...
            <!-- Payment Method -->
            <div class="bg-slate-50 rounded-xl p-3.5 flex items-center justify-between border border-slate-100 mb-6">
        ...
        <div class="mt-8 flex gap-8 text-sm font-bold text-slate-400">
            <a href="{{ route('kasir.dashboard') }}" class="hover:text-primary transition-colors flex items-center gap-2">
                <i class="fa-solid fa-arrow-left text-xs"></i>
                Back to Dashboard
            </a>
            <a href="{{ route('kasir.order') }}" class="hover:text-primary transition-colors flex items-center gap-2">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const renderReceipt = (receipt) => {
            ...
            const totalItems = Number(receipt.totalItems || items.reduce((acc, item) => acc + Number(item.quantity || 0), 0));
        ...
        const loadFromServer = async (id) => {
            ...
            return {
                ...
                totalItems: (order.items || []).reduce((acc, item) => acc + Number(item.quantity || 0), 0),
