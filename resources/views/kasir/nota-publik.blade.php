<x-app-layout title="Digital Receipt #{{ $transaction->order_code }}">
    <div class="min-h-screen bg-slate-50 flex flex-col items-center py-10 px-4">
        <!-- Receipt Container -->
        <div class="bg-white w-full max-w-[400px] rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 overflow-hidden relative">
            ...
            <div class="p-8 sm:p-10">
                ...
                <div class="text-center mb-6">
                    <img src="{{ asset('img/logo-perpus.webp') }}" alt="Logo" class="h-16 mx-auto mb-4 grayscale opacity-90">
                ...
                <div class="space-y-2 text-[12px]">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-slate-800 font-bold tracking-tight">{{ $transaction->paid_at ? $transaction->paid_at->format('d/m/Y H:i') : $transaction->created_at->format('d/m/Y H:i') }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-slate-400 font-bold tracking-widest">Kasir</span>
                        <span class="text-slate-800 font-bold tracking-tight">{{ $transaction->user->name ?? 'Kasir' }}</span>
                    </div>
                </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-1 pr-4">
                            <h4 class="text-[13px] font-black text-slate-800 leading-tight uppercase tracking-tight">{{ $item->product_name }}</h4>
                ...
                <div class="space-y-2.5">
                    <div class="flex justify-between text-[13px] items-center">
                        <span class="text-slate-500 font-bold uppercase tracking-wider">Subtotal ({{ $transaction->items->sum('quantity') }} item)</span>
                        <span class="text-slate-800 font-bold tracking-tight">{{ number_format($transaction->subtotal, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="pt-4 space-y-2">
                        <div class="flex justify-between text-[12px] items-center">
                            ...
                            <span class="text-slate-800 font-bold tracking-tight">{{ number_format($transaction->paid_amount, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-[12px] items-center">
                            <span class="text-slate-400 font-bold uppercase tracking-widest">Kembalian</span>
                            <span class="text-slate-800 font-bold tracking-tight">{{ number_format($transaction->change_amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
        ...
        <!-- Back Link -->
        <a href="https://instagram.com/perpusums" target="_blank" class="mt-8 flex items-center gap-2 text-slate-400 hover:text-primary transition-colors text-xs font-black uppercase tracking-[0.2em]">
